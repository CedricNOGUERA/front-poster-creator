...
import { Modal, Button, Form, Row, Col, Card, Alert, ToggleButton, ButtonGroup } from 'react-bootstrap'
...
const PrintOptionsModalAdvanced: React.FC<PrintOptionsModalProps> = ({
  show,
  onHide,
  templateState,
  canvasRef
  ...
  const [conversionMode, setConversionMode] = useState<'real' | 'preview'>('real')
    ...
    for (const file of Array.from(files)) {
      if (file.type === 'application/pdf') {
        ...
        try {
          ...
          const converterPath = conversionMode === 'real' 
            ? '@/utils/pdfConverterReal' 
            : '@/utils/pdfConverterSimple'
          
          const { convertPDFToSingleImage } = await import(converterPath)
          ...
          {printMode === 'combine' && (
            <>
              ...
              <Row>
                <Col md={6}>
                  <Form.Group className="mb-3">
                    <Form.Label>Mode de conversion des PDFs</Form.Label>
                    <ButtonGroup>
                      <ToggleButton
                        id="conversion-real"
                        type="radio"
                        ...
                        name="conversionMode"
                        value="real"
                        checked={conversionMode === 'real'}
                        onChange={(e) => setConversionMode(e.currentTarget.value as "real" | "preview")}
                      >
                        Contenu réel
                      </ToggleButton>
                      <ToggleButton
                        ...
                        name="conversionMode"
                        value="preview"
                        checked={conversionMode === 'preview'}
                        onChange={(e) => setConversionMode(e.currentTarget.value as "real" | "preview")}
                      >
                        Aperçu visuel
                      </ToggleButton>
                    ...
                    <Form.Text className="text-muted">
                      <strong>Contenu réel :</strong> Extrait le contenu visuel du PDF<br />
                      <strong>Aperçu visuel :</strong> Crée un aperçu avec les informations du fichier
                    </Form.Text>
                  </Form.Group>
                </Col>
                <Col md={6}>
                  <Form.Group className="mb-3">
                    <Form.Label>Mode de redimensionnement</Form.Label>
                    <ButtonGroup>
                      <ToggleButton
                        id="resize-proportional"
                        type="radio"
                        variant="outline-success"
                        name="resizeMode"
                        value="proportional"
                        checked={resizeMode === 'proportional'}
                        onChange={(e) => setResizeMode(e.currentTarget.value as "proportional" | "original" | "fit")}
                      >
                        Proportionnel
                      </ToggleButton>
                      <ToggleButton
                    ...
                    <Form.Text className="text-muted">
                      <strong>Proportionnel :</strong> Redimensionne en gardant les proportions<br />
                      <strong>Dimensions d'origine :</strong> Conserve les dimensions exactes<br />
                      <strong>Ajuster à la page :</strong> Ajuste pour maximiser l'utilisation de l'espace
                    </Form.Text>
                  </Form.Group>
                </Col>
              </Row>
...
export default PrintOptionsModalAdvanced
