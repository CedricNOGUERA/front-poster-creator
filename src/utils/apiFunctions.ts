...
export const _deleteModels = async (ids: number[]) => {
  try {
    ids.map(async (id) => {
      await modelsServiceInstance.deleteModel(id);
    });
  } catch (error) {
    console.error(error);
  }
...
export const _getImagesModels = async (
  categoryId: number,
  ...
  try {
    ...
    const filteredImages = response.data.filter(
      (img: ImagemodelType) => img.categoryId === categoryId,
    );
...
export const _getAllImagesModels = async (
  setImagesModels: React.Dispatch<React.SetStateAction<ImagemodelType[]>>,
) => {
  try {
    const response = await modelsServiceInstance.getModelImage();
...
export const _getCategoryPictures = async (
  ...
  try {
    const result =
      await categoriesServiceInstance.getPicturesByCategory(categoryId);
